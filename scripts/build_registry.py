...
Layout open-conto-annuale:
- dataset.yml in ``datasets/*/`` (dataset principali) e ``support/*/`` (anagrafiche seed);
- parquet e run records in ``out/data/`` (root dichiarato nei dataset.yml);
- GCS: ``gs://dataciviclab-{clean,mart}/conto-annuale/{slug}/{year}/`` (layout year).
...
def main() -> int:
    ...
    layout = RepoLayout(
        repo_root=ROOT,
        dataset_dirs=("datasets", "support"),
        source_repo="dataciviclab/open-conto-annuale",
    )
    contract = PathContract(prefix="conto-annuale", clean_layout="year", mart_layout="year")
